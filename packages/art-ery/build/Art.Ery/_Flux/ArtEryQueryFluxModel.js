...
(function() {
  var ArtEryQueryFluxModel, FluxModel, KeyFieldsMixin, Promise, arrayWith, arrayWithElementReplaced, arrayWithout, decapitalize, defineModule, eq, formattedInspect, isArray, isFunction, isString, log, merge, missing, pending, propsEq, ref, ref1, select, success, upperCamelCase,
  ...
  KeyFieldsMixin = require("art-ery").KeyFieldsMixin;

  FluxModel = require('@art-suite/art-flux').FluxModel;
  ...
  defineModule(module, ArtEryQueryFluxModel = (function(superClass) {
    ...
    ArtEryQueryFluxModel.prototype.dataUpdated = function(queryKey, singleRecordData) {
      return this._updateFluxStoreIfExists(queryKey, singleRecordData);
    };

    ArtEryQueryFluxModel.prototype.dataDeleted = function(queryKey, singleRecordData) {
      return this._updateFluxStoreIfExists(queryKey, singleRecordData, true);
    };

    ArtEryQueryFluxModel.prototype._updateFluxStoreIfExists = function(queryKey, singleRecordData, wasDeleted) {
      if (this.getModelRecord(queryKey)) {
        return this.updateModelRecord(queryKey, (function(_this) {
          return function(oldFluxRecord) {
            var merged;
            if (merged = _this.localMerge(oldFluxRecord.data, singleRecordData, wasDeleted)) {
              return merge(oldFluxRecord, {
                data: _this.localSort(merged)
              });
            } else {
              return oldFluxRecord;
            }
          };
        })(this));
      }
