...
(function() {
  ...
  ref2 = require('art-ery'), KeyFieldsMixinCoffee = ref2.KeyFieldsMixinCoffee, PipelineRegistry = ref2.PipelineRegistry, pipelines = ref2.pipelines;

  ref3 = require('@art-suite/art-models'), ArtModel = ref3.ArtModel, models = ref3.models;
  ...
  defineModule(module, ArtEryFluxModel = (function(superClass) {
    ...
    ArtEryFluxModel.createModel = function(pipeline) {
      var AnonymousArtEryArtModel, aliases, hotReloadKey, name;
      ...
      return createWithPostCreate(AnonymousArtEryArtModel = (function(superClass1) {
        var ucName;

        extend(AnonymousArtEryArtModel, superClass1);

        function AnonymousArtEryArtModel() {
          return AnonymousArtEryArtModel.__super__.constructor.apply(this, arguments);
        }

        AnonymousArtEryArtModel._name = ucName = upperCamelCase(name);

        if (pipeline.keyFields) {
          AnonymousArtEryArtModel.keyFields(pipeline.keyFields);
        }

        AnonymousArtEryArtModel.pipeline(pipeline);

        if (aliases) {
          AnonymousArtEryArtModel.aliases(aliases);
        }

        AnonymousArtEryArtModel.getHotReloadKey = function() {
          return hotReloadKey;
        };

        return AnonymousArtEryArtModel;
    ...
    ArtEryFluxModel.prototype.dataUpdated = function(key, data) {
      ...
      this.updateModelRecord(key, function(oldModelRecord) {
        return merge(oldModelRecord, {
          data: merge(oldModelRecord.data, data)
        });
      });
