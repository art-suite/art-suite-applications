...
(function() {
  var BaseObject, CommunicationStatus, DatabaseFilters, Filter, Filters, Pipeline, PipelineRegistry, Promise, Request, RequestResponseBase, Response, Session, arrayToTruthMap, cleanStackTrace, compactFlatten, config, currentSecond, decapitalize, defineModule, each, escapeRegExp, formattedInspect, getCodeWords, getDetailedRequestTracingEnabled, getEnv, inspect, inspectedObjectLiteral, isArray, isClass, isFunction, isPlainArray, isPlainObject, isString, log, lowerCamelCase, merge, mergeInto, missing, normalizeFieldProps, object, peek, plainObjectsDeepEq, pluralize, prefetchedRecordsCache, present, pushIfNotPresent, ref, reverseForEach, success, toDate, w,
  ...
  defineModule(module, Pipeline = (function(superClass) {
    ...
    Pipeline.prototype.unsubscribe = function(subscriber) {
      return removeFirstMatch(this._subscribers, subscriber);
    };

    Pipeline.prototype.dataUpdated = function(key, data) {
      return this._sendDataSubscriptionEvent("update", key, data);
    };

    Pipeline.prototype.dataDeleted = function(key, data) {
      return this._sendDataSubscriptionEvent("delete", key, data);
    };

    Pipeline.prototype._sendDataSubscriptionEvent = function(eventType, key, data) {
      var dataArray, i, j, len, len1, ref1, ref2, results, results1, subscriber;
      if (isArray(dataArray = data)) {
        ...
        for (i = 0, len = ref1.length; i < len; i++) {
          ...
          results.push((function() {
            var j, len1, results1;
            results1 = [];
            for (j = 0, len1 = dataArray.length; j < len1; j++) {
              data = dataArray[j];
              results1.push(subscriber(eventType, key, data));
            }
            return results1;
          })());
        }
    ...
    Pipeline.prototype.isUpdateRequestType = function(actionType) {
      return /^(create|update)/.test(actionType);
    };

    Pipeline.prototype.isDeleteRequestType = function(actionType) {
      return /^delete/.test(actionType);
    };
    ...
    Pipeline._normalizeQuery = function(queryName, options) {
      var dataToKeyString, toKeyString;
      ...
      dataToKeyString = options.dataToKeyString;
      toKeyString = dataToKeyString ? function(dataOrKey) {
        if (isPlainObject(dataOrKey)) {
          return dataToKeyString(dataOrKey);
        } else {
          return dataOrKey;
        }
      } : function(key) {
        ...
        return request.requireServerOriginOr(isPublicRequestType, "to issue non-public requests").then((function(_this) {
          ...
          return function(response) {
            ...
            if (response.isSuccessful) {
              ...
              if (response.isUpdateRequest()) {
                _this.dataUpdated(key != null ? key : _this.toKeyString(data), data);
              }
              if (response.isDeleteRequest()) {
                return _this.dataDeleted(key != null ? key : _this.toKeyString(data), data);
              }
            }
          };
        })(this));
