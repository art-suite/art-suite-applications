define [
  ...
  suite "Art.Text.Layout", ->
    test "new", ->
      layout = new Art.Text.Layout oneLineText,
    ...
    logBitmapTest "blank lines", ->
      layout = new Art.Text.Layout "Hi\n\nthere."
      assert.eq 3, layout.lineCount
      layout.toBitmap()

    logBitmapTest "layoutMode: 'textualBaseline'", ->
      layout = new Art.Text.Layout "Going somewhere?", {fontSize: 16}, layoutMode: 'textualBaseline'
      b = layout.toBitmap()
    ...
    test "precision", ->
      layout = new Art.Text.Layout "hi", fontFamily:"Impact", fontSize:64, layoutMode:"tight"
      bitmap = layout.toBitmap()
      log bitmap
    ...
    test "multi-lined", ->
      layout = new Art.Text.Layout "Testing multi-line layout.\nLine two."
      log layout.toBitmap()
      assert.eq (fragment.layoutArea.x for fragment in layout.fragments), [0, 0]

    test "center alignment", ->
      layout = new Art.Text.Layout "Testing center alignment.\ncentered", {}, align:"center", 200
      assert.eq
      log layout.toBitmap()
    ...
    test "layoutAreaWidth basic", ->
      layout = new Art.Text.Layout oneLineText + " " + oneLineText, {}, null, 200
      log layout.toBitmap()
      assert.within layout.size.x, 190, 200

    test "layoutAreaWidth with layoutMode: 'textualBaseline'", ->
      layout = new Art.Text.Layout oneLineText + " " + oneLineText, {fontSize: 16}, leading:1, layoutMode: 'textualBaseline', 200
      log layout.toBitmap()
      assert.eq layout.size.y, 16 * 4 - 4

    test "alignmentWidth basic", ->
      layout = new Art.Text.Layout oneLineText + " " + oneLineText, {}, {}, 200
      log layout.toBitmap()
      assert.within layout.size.x, 195, 196

    test "layoutAreaWidth and manual wrap have same height", ->
      wwLayout = new Art.Text.Layout oneLineText + " " + oneLineText, {}, null, 200
      log wordWrapedLayout: wwLayout.toBitmap()

      manualLayout = new Art.Text.Layout "Quickly the brown fox jumps\nover the lazy dog. Quickly the\nbrown fox jumps over the lazy\ndog", {}
    ...
    test "layoutAreaWidth long word single", ->
      ...
      layout = new Art.Text.Layout longWord, {}, null, 100
