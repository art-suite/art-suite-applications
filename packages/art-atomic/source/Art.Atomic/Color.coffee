  ...
  _initFromString: (string) ->
    ...
    # rgba(red, green, blue, alpha) - rgb values are 0-255, alpha values are 0.0 - 1.0
      ...
      unless clr = colorNamesMap[lcString]
        return log.warn @parseError = "WARNING: Color.parse failure. Unknown rgbColor name: #{inspect string}"
      ...
      return log.warn @parseError = "WARNING: Color.parse failure for #{inspect string}"
