# TODO:
#   Intersection, Union, and Contains should all avoid creating new Points or Rectangles if possible
define [
  'art.foundation'
  './namespace' # required so we can break the circular dependency between Matrix and Rectangle
  './base'
  './point'
  ], (Foundation, Atomic, AtomicBase, Point) ->
  ...
  class Rectangle extends AtomicBase
    @rect: rect = (a, b, c, d) ->
    ...
    @getter
      ...
      locationMatrix: -> Atomic.Matrix.translate @x, @y
      size: -> new Point @w, @h
      # x: -> @x
      # y: -> @y
      # w: -> @w
    ...
    inspect: (inspector)->
      if inspector?.put
        inspector.put @inspect2()
      else
        @inspect2()
    inspect2: -> "rect(#{@x}, #{@y}, #{@w}, #{@h})"
