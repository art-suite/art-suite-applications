...
suite "Art.Flux.Models.VolatileModel", ->
  ...
  test "load missing record returns failure/missing", (done) ->
    reset()
    ...
    fluxStore.subscribe "bar", "foo", (fluxRecord) ->
      if fluxRecord.status != "pending"
        assert.eq fluxRecord, status: missing, key: "foo", modelName: "bar"
        done()

  test "load after put returns success", (done)->
    reset()
    class Bar extends VolatileModel
      @register()

    fluxStore.subscribe "bar", "foo", (fluxRecord) ->
      # NOTE: the initial subscription result is missing since it doesn't exist yet...
      return unless fluxRecord.status != "pending" && fluxRecord.status != missing
  ...
  test "load after post returns success", (done)->
    reset()
    class Bar extends VolatileModel
      @register()
  ...
  test "field type - email valid", (done)->
    ...
    ModelRegistry.models.user.post email:"  frank@GMail.com  ", (statusRecord) ->
      if statusRecord.status != "pending"
        assert.eq statusRecord, status: success, data: {email: "frank@gmail.com", id: "0"}, key: "0", modelName: "user"
        done()
